...
//if -d is set and it's not a directory then skip it
	return 0;
     }
} else {
   if (!opts.dirsonly) {
// if its not a directory and -d is not set, then print it
	print_path_info(finfo); 
   }
// if -d is set then do nothing
   return 0;
}
...
exit:
    if (file_list != NULL) {
	free_file_list(&file_list, file_count); // Free the file list
    }
    if (dir != -1) {
	close(dir); // Close the directory file descriptor
    }
  cur_dir = sav_dir;
  return errno ? -1 : 0;
}
...
filecmp(void const *_lhs, void const *_rhs)
{
  ...
  switch (opts.sort) {
    case NONE:
      ...
      break;
    case ALPHA:
      retval = strcoll(((struct fileinfo *)_lhs)->path, ((struct fileinfo *)_rhs)->path);
      break; 
    case RALPHA:
